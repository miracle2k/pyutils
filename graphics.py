"""Graphic and Imaging related functionality for Python.
"""
...
def make_thumbnail(image, new_width, new_height, mode="crop",
    ...
    Parameters:
        ...
        * ``save_to``:
            If this is given, not only is the thumbnail image returned,
            but the function will also attempt to save it at the path
            specified. Can also be a function (or lambda) that returns
            the path. It will be called with three parameters: image
            (the Image object), and ``new_width`` and ``new_height``.
            For example, you can use image.format to determine the
            extension.

        * ``force``:
            If this is False (default: True), the function will attempt
            to compare the timestamps of the orginal and thumbnail images,
            and only recreate the image if the thumbnail is older than
            the image.
    ...
    # resize the image according to requested mode
    if mode == "crop":
